...
func (d GoogleToken)TokenInfo(req *http.Request)(GoogleTokenInfo , error){
	...
	err := goauth.CallAPI(req,"GET", "https://www.googleapis.com/oauth2/v3/tokeninfo", values, &ai)	
	return ai,err
}
...
func (d GoogleToken)AccountInfo(req *http.Request)(GoogleAccountInfo , error){
	...
	values := make(url.Values)
	values.Add("access_token",d.AccessToken)
	err := goauth.CallAPI(req,"GET", "https://www.googleapis.com/oauth2/v3/userinfo", values, &ai)	
	return ai,err
}
...
func GoogleRecieve(res http.ResponseWriter, req *http.Request, redirect ,clientID, secretID string, token *GoogleToken) error {
	resp, err := goauth.RequiredRecieve(res,req, clientID, secretID, redirect, "https://www.googleapis.com/oauth2/v4/token")
	if err != nil { 
		return err 
	}
